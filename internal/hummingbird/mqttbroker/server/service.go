...
type Publisher interface {
	...
	Publish(message *mqttbroker.Message)
}
...
// ClientService provides the ability to query and close alertclient.
type ClientService interface {
	IterateSession(fn session.IterateFn) error
	GetSession(clientID string) (*mqttbroker.Session, error)
...
type SubscriptionService interface {
	...
	Subscribe(clientID string, subscriptions ...*mqttbroker.Subscription) (rs subscription.SubscribeResult, err error)
